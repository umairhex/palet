...
<template>
  <section class="w-full bg-impact-card py-40 px-6 md:px-12 border-t border-slate-50">
    <div class="max-w-7xl mx-auto">
      <div class="flex flex-col items-center text-center mb-24 space-y-6">
        <h2
          class="text-4xl md:text-5xl lg:text-6xl font-black text-slate-900 tracking-tight leading-[1.1]"
        >
          Trusted by the <br />
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div
          ...
          :key="index"
          class="bg-[#fcfdfe] rounded-[2.5rem] p-10 md:p-14 border border-slate-200 flex flex-col justify-between"
        >
          <div class="space-y-10">
            <div class="flex flex-col gap-6">
              <div class="flex gap-1">
                <Star v-for="i in 5" :key="i" class="size-4 fill-emerald-500 text-emerald-500" />
              </div>
              <div class="flex flex-wrap gap-2">
                <span
                  v-for="tag in testimonial.tags"
                  :key="tag"
                  class="text-[10px] font-black uppercase tracking-widest px-3 py-1 bg-white border border-slate-200 rounded-full text-slate-400"
                >
                  {{ tag }}
                </span>
              </div>
            ...
            <p class="text-2xl text-slate-800 leading-relaxed font-medium tracking-tight">
              "{{ testimonial.quote }}"
            </p>
          </div>
          ...
          <div class="mt-16 pt-8 border-t border-slate-100 flex justify-between items-end">
            <div class="flex flex-col gap-1">
              ...
              <span class="text-xs font-medium uppercase tracking-[0.15em] text-slate-400">
                {{ testimonial.role }}
              </span>
            </div>
            <span class="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-600/40">{{
              testimonial.company
            }}</span>
          </div>
        </div>
